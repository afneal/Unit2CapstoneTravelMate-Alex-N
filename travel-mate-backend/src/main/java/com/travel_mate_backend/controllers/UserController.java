...
public class UserController {
    ...
    public ResponseEntity<?> addNewUser(@RequestBody @Valid UserDTO userData) { //RequestBody: take JSON payload and convert to UserDTO object called userData
                                                                                //valid: to work with @NotBlank etc in dto
            if (userRepository.existsByUsername(userData.getUsername())) { //custom search method in userRepository, if exists then send ErrorObject
                return new ResponseEntity<>(new ErrorObject(409, "Username already exists", "CONFLICT"), HttpStatus.CONFLICT);//409
            }
                UserServices userServices = new UserServices();

                //Create new user with (username, hashedPass)
                User user = new User(userData.getUsername(), userServices.hashPassword(userData.getPassword()));
                userRepository.save(user); //save new user to db, ID auto generated by hibernate from @GeneratedValue in user entity
    ...
    public ResponseEntity<?> loginUser(@RequestBody @Valid UserDTO userData) { //RequestBody to convert the JSON data(from the HTTP text input) into a UserDTO object(userData)
        ...
        UserServices userServices = new UserServices();
        if (user != null && userServices.checkPassword(user, userData.getPassword())) { //checkPassword method from user services
                                                                                    // compares plain password from userData (dto) with stored hash
